...
1.1.0 额外读取上游的 `stream-fadein.js`，配合 morphdom 2.7.7 检查原生文字分段与节点复用。断言新增正文立即完整存在、动画延迟为 0、默认时长为 160ms、已有片段节点和动画对象保持不变、减少动态效果生效。布局夹具包括头像、名字和主题强制按钮装饰，1.2.0 改为断言正文保留原生头像栏，展开前后正文横坐标和宽度相同、仅纵坐标增加，折叠槽高度为 0，COT 位于正文左侧，思考按钮没有阴影或描边。
...
1.2.0 还检查原生 initHandleMessage(reset) 在首 token 前立即移除面板和标签且不删除存档、不同候选替换内容并恢复收起状态、无思考候选清空显示，以及克隆消息中旧标签和容器不会重复。
