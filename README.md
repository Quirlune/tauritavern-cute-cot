...
- 采用大圆角一体结构和浅粉配色，移除外描边、阴影，限制主题对思考按钮的立体装饰。
- 恢复正文原有位置和宽度，移除 1.1.0 的铺满布局及其设置。思考完成后缩为头像下方的竖向 COT 标签，正文上方不留下折叠栏。
- 点击 COT 时面板向右下方展开，正文仅向下让位；再次收起后恢复。左右切换候选回复时同步清空旧面板，新候选尚未输出思考也不残留；旧候选记录仍可回看。
- 正文默认启用酒馆原生流式淡入，采用 **160 毫秒、零延迟**的短动画，从部分可见过渡到完全可见。可调为 80–260 毫秒。
- 不建立逐字队列，不等待上一段动画结束，不改模型请求、生成参数和酒馆的流式刷新率。服务器本身的分块间隔仍取决于接口与网络。
- 平滑选项会启用酒馆的原生“流式文字淡入”；关闭该选项或停用扩展时，恢复本扩展启用前的原生设置。系统启用“减少动态效果”时不播放正文淡入。

更新后重新加载一次，自动恢复原有正文布局，并采用侧边收起方式。正文平滑设置会保留。自定义配色会保留，仅旧版默认粉色会迁移到新的浅粉色。
